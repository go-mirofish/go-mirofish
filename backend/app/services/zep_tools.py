...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.historical_facts:
            text_parts.append(f"\n### Historical or expired facts (evolution trail)")
...
class ZepToolsService:
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are an expert at analytical decomposition. Break a complex question into several sub-questions that can be observed independently inside the simulated world.

Requirements:
1. Each sub-question must be concrete enough to map to agent behavior or events.
2. The set should cover multiple dimensions of the original question.
3. The sub-questions must remain relevant to the simulation scenario.
4. Return JSON in this form: {"sub_queries": ["sub-question 1", "sub-question 2", ...]}"""
...
Return the sub-question list as JSON."""
        ...
        try:
            ...
            return [
                query,
                f"Who are the main actors in {query}?",
                f"What causes and impacts shape {query}?",
    ...
    def interview_agents(
        ...
        if not profiles:
            ...
            result.summary = "No interviewable agent profile file was found."
            return result
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Answer the following questions directly using your persona, memories, and prior actions.\n"
            "Response requirements:\n"
            "1. Answer in plain natural language without using any tools\n"
            "2. Do not return JSON or tool-call syntax\n"
            "3. Do not use Markdown headings\n"
            "4. Answer each question in order, starting with 'Question X:'\n"
            "5. Separate each answer with a blank line\n"
            "6. Each answer should contain meaningful content with at least 2-3 sentences\n\n"
        )
        ...
        try:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may have been closed."
            return result
        except Exception as e:
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are an interview planner. Select the best simulated agents for a given interview objective.

Selection criteria:
1. The agent's identity or profession matches the topic
2. The agent is likely to hold a useful or distinctive viewpoint
3. The final set should cover multiple perspectives
4. Prefer agents directly connected to the event
...
Select up to {max_agents} agents and explain why they are the best interview targets."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional interviewer. Generate 3-5 deep interview questions for the selected agents.

Requirements:
1. Use open-ended questions that encourage detailed answers
2. Questions should work across different roles
3. Cover facts, opinions, and feelings
4. Keep the tone natural and interview-like
5. Keep each question concise
6. Ask directly without extra background prefixes

Return JSON: {"questions": ["Question 1", "Question 2", ...]}"""

        user_prompt = f"""Interview requirement: {interview_requirement}

Scenario background: {simulation_requirement if simulation_requirement else "Not provided"}

Interview target roles: {', '.join(agent_roles)}
...
Generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        quote_instruction = "Use the locale-appropriate quotation marks when quoting interviewees."
        system_prompt = f"""You are a professional editor. Summarize the interview responses into a concise interview brief.

Requirements:
1. Extract the main viewpoints
2. Identify areas of agreement and disagreement
3. Highlight strong quotes
4. Stay objective and neutral
5. Keep the summary under about 1000 words
