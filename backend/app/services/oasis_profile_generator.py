"""Generate OASIS-compatible agent profiles from graph entities."""
...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Zep hybrid retrieval completed: {entity_name}, got {len(results['facts'])} facts and {len(results['node_summaries'])} related nodes")
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        logger.warning(f"JSON repair failed; returning a fallback structure")
        return {
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        base_prompt = "You are a social-media persona specialist. Generate detailed, realistic personas for opinion simulation. Return valid JSON only, and do not include unescaped newlines in string values."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social-media persona for this entity and keep it grounded in the provided evidence.
...
1. bio: concise social profile text, around 200 characters
2. persona: a detailed plain-text persona description, including:
   - basic facts such as age, profession, education, and location
   - background and relationship to the event
   - personality traits and emotional style
   - social-media habits, posting preferences, and interaction style
   - likely viewpoints and triggers
   - memorable traits, interests, and signature behaviors
   - event memory and prior actions tied to the scenario
3. age: integer
4. gender: one of "male" or "female"
5. mbti: MBTI type such as INTJ or ENFP
6. country: country or region name in the report language
7. profession: profession label
8. interested_topics: array of relevant topics

Important:
- all values must be strings, numbers, or arrays of strings
- persona must be one continuous paragraph without embedded newlines
- {get_language_instruction()} (the `gender` field must stay in English)
- keep the content aligned with the entity evidence
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed institutional or group account persona grounded in the provided evidence.
...
Return JSON with these fields:
1. bio: concise official account bio, around 200 characters
2. persona: a detailed plain-text account description, including:
   - core organizational facts, role, and mission
   - account positioning, audience, and function
   - communication style and topic boundaries
   - publishing habits and active periods
   - stance on the core issue and crisis behavior
   - operational habits and institutional memory
   - the organization’s connection to the scenario and prior reactions
3. age: fixed integer `30`
4. gender: fixed string `"other"`
5. mbti: an MBTI style descriptor for the account voice
6. country: country or region name in the report language
7. profession: institutional function label
8. interested_topics: array of focus areas

Important:
- all values must be strings, numbers, or arrays of strings
- persona must be one continuous paragraph without embedded newlines
- {get_language_instruction()} (the `gender` field must stay in English and be `"other"`)
- age must be the integer `30`
- the account voice should stay consistent with the organization identity
"""
    ...
    def generate_profiles_from_entities(
        ...
        completed_count = [0]  # use a list so the closure can update the counter
        lock = Lock()
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        ...
                        logger.info(f"[{current}/{total}] Generated persona successfully: {entity.name} ({entity_type})")
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                row = [
                    ...
                    profile.user_name,      # username: account name
                    user_char,              # user_char: full persona text for the internal model
