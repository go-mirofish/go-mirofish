...
INTERVIEW_PROMPT_PREFIX = "Using your persona, memories, and prior actions, answer me directly in plain text without calling any tools:"
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        logger.debug(f"Checking prepare status: {simulation_id}, status={status}, config_generated={config_generated}")
        ...
        if status in prepared_statuses and config_generated:
            ...
            if status == "preparing":
                try:
                    ...
                    logger.warning(f"Failed to auto-update simulation status: {e}")
    ...
    try:
        ...
        if not force_regenerate:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} is not prepared; starting prepare task")
...
def get_simulation_history():
    ...
    try:
        ...
        logger.error(f"Failed to load simulation history: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_comments(simulation_id: str):
    """Get simulated comments (Reddit only)."""
    try:
...
def interview_agent():
    """Interview a single agent while the simulation environment is running."""
    try:
        data = request.get_json() or {}
...
def get_interview_history():
    """Load interview history from the simulation database."""
    try:
        data = request.get_json() or {}
...
def get_env_status():
    """Return whether the simulation environment is alive and interview-ready."""
    try:
        data = request.get_json() or {}
