...
- the runtime is a local-first Go gateway
...
- [gateway/cmd/mirofish-hybrid](https://github.com/go-mirofish/go-mirofish/tree/main/gateway/cmd/mirofish-hybrid) (`api-smoke` subcommand for a full API walk)
...
2. Startup proof
   - `make up` (or an equivalent `docker compose up` for the dev gateway image)
   - `curl http://127.0.0.1:3000/health`
   - `curl http://127.0.0.1:3000/ready`

3. Benchmark proof
   - `cd gateway && go run ./cmd/mirofish-hybrid api-smoke -base-url http://127.0.0.1:3000` (optional full-API check; requires working LLM/Zep as usual)
...
cd gateway && go run ./cmd/mirofish-hybrid api-smoke -base-url http://127.0.0.1:3000
```
