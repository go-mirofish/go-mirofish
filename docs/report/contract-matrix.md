...
> **Historical:** Written during the Flask → Go migration. The product path is **Go-only** today; the “Flask route” column describes the **legacy** contract the gateway replaced.
...
- Legacy Python/Flask backend routes (removed from the runtime; kept here for contract traceability)
- Frontend callers at the time of the matrix
- Gateway aliases required where those surfaces differ

## Matrix

| Capability | Locked benchmark contract | Legacy Flask route (removed) | Frontend caller (at matrix time) | Gateway policy |
| --- | --- | --- | --- | --- |
