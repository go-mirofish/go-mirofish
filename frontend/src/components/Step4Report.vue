...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const histMatch = text.match(/Historical or expired facts:\s*(\d+)/)
    ...
    const histSection = text.match(/### Historical or expired facts[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Interviewed agents:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Select\s*([^(\n]+)(?:\s*\(index\s*=?\s*\d+\))?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No reply from this platform)' || t === '[No reply]'
    }
...
.header-dot {
  ...
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--doc-text) 18%, transparent);
  margin-right: 10px;
  flex-shrink: 0;
...
@keyframes pulse-dot {
  ...
  50% {
    box-shadow: 0 0 0 5px color-mix(in srgb, var(--doc-text) 12%, transparent);
  }
}
...
.left-panel:hover::-webkit-scrollbar-thumb {
  background: color-mix(in srgb, var(--doc-text) 16%, transparent);
}

.left-panel::-webkit-scrollbar-thumb:hover {
  background: color-mix(in srgb, var(--doc-text) 28%, transparent);
}
...
.section-header-row.clickable:hover {
  background-color: var(--doc-upload-surface);
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--doc-code-bg); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--doc-border);
  --wf-divider: var(--doc-code-bg);

  --wf-active-bg: var(--doc-workbench-mid);
  --wf-active-border: var(--doc-text);
  --wf-active-dot: var(--doc-text);
  --wf-active-text: var(--doc-text);

  --wf-done-bg: var(--doc-upload-surface);
  --wf-done-border: var(--doc-border);
  --wf-done-dot: var(--doc-badge-ok-fg);

  --wf-muted-dot: var(--doc-dashed);
  --wf-todo-text: var(--doc-muted);
}
...
/* Responsive: when the available width is narrow (e.g. split mode),
   stack panels to avoid clipping due to parent overflow hidden. */
...
.metric-pill.pill--completed {
  ...
  border-color: color-mix(in srgb, var(--doc-badge-ok-fg) 32%, var(--doc-border));
  color: var(--doc-badge-ok-fg);
}
...
.timeline-footer {
  ...
  margin-top: 10px;
  padding-top: 10px;
  border-top: 1px solid var(--doc-code-bg);
}
...
.status-message.success {
  background: var(--doc-badge-ok-bg);
  border-color: color-mix(in srgb, var(--doc-badge-ok-fg) 28%, var(--doc-border));
  color: var(--doc-badge-ok-fg);
}
...
.section-tag.completed {
  background: var(--doc-badge-ok-bg);
  border: 1px solid color-mix(in srgb, var(--doc-badge-ok-fg) 28%, var(--doc-border));
}
...
.tool-badge.tool-gray {
  background: linear-gradient(135deg, var(--doc-upload-surface) 0%, var(--doc-code-bg) 100%);
...
:deep(.interview-display .expand-answer-btn) {
  ...
  background: transparent;
  border: none;
  border-bottom: 1px dotted var(--doc-dashed);
  border-radius: 0;
  font-size: 11px;
  font-weight: 500;
  color: var(--doc-muted);
...
:deep(.insight-display .rel-target) {
  padding: 4px 8px;
  background: var(--doc-surface);
  border: 1px solid var(--doc-dashed);
  border-radius: 4px;
  font-size: 11px;
  font-weight: 500;
  color: var(--doc-text);
