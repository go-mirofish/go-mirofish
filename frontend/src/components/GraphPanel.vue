...
.graph-panel {
  ...
  background-image: radial-gradient(var(--doc-border, #d0d0d0) 1.5px, transparent 1.5px);
...
.panel-header {
  ...
  background: linear-gradient(
    to bottom,
    color-mix(in srgb, var(--doc-surface, #fff) 92%, transparent),
    transparent
  );
  pointer-events: none;
}
...
.tool-btn:hover {
  ...
  border-color: var(--doc-border, #ccc);
}
...
.graph-legend {
  ...
  left: 24px;
  z-index: 10;
  background: color-mix(in srgb, var(--doc-surface, #fff) 92%, transparent);
  padding: 12px 16px;
  border-radius: 8px;
  border: 1px solid var(--doc-border, #eaeaea);
...
.slider {
  ...
  right: 0;
  bottom: 0;
  background-color: var(--doc-border, #e0e0e0);
  border-radius: 22px;
  transition: 0.3s;
}
